...
const NAV_ITEMS = [
  { to: '/dashboard', icon: '🏠', label: 'Carbon HQ' },
  { to: '/calculator', icon: '🧮', label: 'Calculator' },
  { to: '/actions', icon: '⚡', label: 'Action Hub' },
...
export function Sidebar() {
  ...
  return (
    <aside className="sidebar">
      ...
      {/* Score pill */}
      {latest && (
        <div style={{ marginBottom: 24, padding: '12px 14px', background: 'rgba(0,255,136,0.06)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(0,255,136,0.15)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.4rem', color: '#00FF88' }}>
            {(latest.totalKgCO2e / 1000).toFixed(1)}
            <span style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginLeft: 4 }}>t CO₂e/yr</span>
          </div>
          <div style={{ height: 4, background: 'rgba(255,255,255,0.06)', borderRadius: 999, marginTop: 8, overflow: 'hidden' }}>
            <motion.div style={{ height: '100%', background: 'linear-gradient(90deg, #00FF88, #0DCCB0)', borderRadius: 999 }}
              initial={{ width: 0 }} animate={{ width: `${Math.min((latest.totalKgCO2e / 20000) * 100, 100)}%` }}
